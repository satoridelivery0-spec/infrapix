...
// ======================
// TECLADO PRINCIPAL (igual ao print)
// ======================
...
bot.start(async (ctx) => {
  ...
  if (!existingUser) {
    ...
    const startPayload = ctx.startPayload // ex: 8459593309
    let sponsorId = null
...
bot.hears('💰 DEPOSITAR', async (ctx) => {
  await ctx.reply('Digite o valor que deseja depositar (ex: 50.00):')
  // Depois vamos melhorar essa parte
})

bot.hears('💸 SACAR', async (ctx) => {
  await ctx.reply('Digite o valor que deseja sacar:')
})
